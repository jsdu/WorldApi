...
@objc public final class HTMap: NSObject, HTConsumerClientDelegate, HTViewInteractionInternalDelegate {
    ...
    func setUpHeroMarker(userId: String, coordinates: [CLLocationCoordinate2D]) {
        ...
        let action = user?.actions?.last as! HyperTrackAction
...
extension HTMap : MapCustomizationDelegate{
    ...
    func annotationView(_ mapView: MKMapView, annotation: HTMapAnnotation) -> MKAnnotationView?
    {
        ...
        if(annotation.type == HTConstants.MarkerType.HERO_MARKER){
            return self.customizationDelegate?.heroMarkerViewForActionID?(map: self, actionID: (annotation.action?.id)!)
        }
        else if (annotation.type == HTConstants.MarkerType.DESTINATION_MARKER){
            return self.customizationDelegate?.expectedPlaceMarkerViewForActionID?(map: self, actionID: (annotation.action?.id)!)
        }
    ...
    func imageView(_ mapView: MKMapView, annotation: HTMapAnnotation) -> UIImage?{
        ...
        if(annotation.type == HTConstants.MarkerType.HERO_MARKER){
            return self.customizationDelegate?.heroMarkerImageForActionID?(map: self, actionID: (annotation.action?.id)!)
        }
        else if (annotation.type == HTConstants.MarkerType.DESTINATION_MARKER){
            return self.customizationDelegate?.expectedPlaceMarkerImageForActionID?(map:self, actionID: (annotation.action?.id)!)
        }
        return nil
    }
